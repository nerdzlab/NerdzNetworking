...
public class ExecutionOperation<T: Request>: DispatchOperation {
    ...
    public typealias StartCallback = (DispatchOperation) -> Void
    
    var internalOperation: DispatchOperation?
    ...
    init(decoder: JSONDecoder = JSONDecoder(), responseQueue: DispatchQueue = OperationQueue.current?.underlyingQueue ?? .main, retryOnFail: Bool = true) {
    ...
    func callOnStart(with requestOperation: DispatchOperation) {
        responseQueue.async { [weak self] in
            for closure in self?.onStart ?? [] {
                closure(requestOperation)
            }
        }
    }
